...
class Email
{
    ...
    */
    public static function send(string $email, string $title, string $body): array
    {
        ...
        if (!$validate->check($config)) return ['status' => 'error', 'message' => lang('the system does not have a mailbox configured')];
        ...
        $mail->IsHTML(true);
        $message = $mail->send();
        return $message === true ? ['status' => 'success', 'message' => lang('sent successfully')] : ['status' => 'error', 'message' => lang($message)];
    }
}